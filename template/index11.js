...
function fire(){
    ...
    bodyDefBullet.position.x = (user.GetPosition().x + 0.8*Math.cos(user.GetAngle()));
    bodyDefBullet.position.y = (user.GetPosition().y + 0.8*Math.sin(user.GetAngle()));
    var bullet = world.CreateBody(bodyDefBullet).CreateFixture(fixDef);
    bullet.GetBody().SetUserData({id: "bullet"});
    console.log(bullet)
    velocity = 1;
    bullet.GetBody().ApplyImpulse(new b2Vec2(velocity*Math.cos(user.GetAngle()), velocity*Math.sin(user.GetAngle())), bullet.GetBody().GetWorldCenter());
...
listener.BeginContact = function(contact) {
    ...
    if (contact.GetFixtureA().GetBody().GetUserData() != null && contact.GetFixtureB().GetBody().GetUserData() != null) {
        if (contact.GetFixtureA().GetBody().GetUserData().id == "user2" && contact.GetFixtureB().GetBody().GetUserData().id == "bullet" || contact.GetFixtureA().GetBody().GetUserData().id == "bullet" && contact.GetFixtureB().GetBody().GetUserData().id == "user2") {
...
listener.EndContact = function(contact) {
         if (contact.GetFixtureA().GetBody().GetUserData() != null && contact.GetFixtureB().GetBody().GetUserData() != null) {
        if (contact.GetFixtureA().GetBody().GetUserData().id == "user2" && contact.GetFixtureB().GetBody().GetUserData().id == "bullet" || contact.GetFixtureA().GetBody().GetUserData().id == "bullet" && contact.GetFixtureB().GetBody().GetUserData().id == "user2") {
	    positionX = contact.GetFixtureA().GetBody().GetPosition().x;
	    positionY = contact.GetFixtureA().GetBody().GetPosition().y;
	}
